...
tsi_thread = threading.Thread(
    ...
    args=(tsi, data_queue, stop_event)
)
...
while time.monotonic()-tstart<duration:
    time.sleep(0.01)
...
while not data_queue.empty():
    flow.extend(data_queue.get())
print("len = ", len(flow), "expected = ", 1000*duration/4)
